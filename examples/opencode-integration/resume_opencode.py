...
"""Demonstrate OpenCode coding-agent session persistence across pause/resume.
...
DEFAULT_OPENCODE_STATE_DIR = "/root/.pi/agent"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--name",
        default="cube-opencode-agent-resume",
        help="OpenCode session name reused across both turns to keep the conversation.",
    )
    parser.add_argument(
        "--opencode-state-dir",
        default=os.environ.get("OPENCODE_AGENT_DIR", DEFAULT_OPENCODE_STATE_DIR),
    ...
    parser.add_argument(
        "--exec-timeout",
        type=int,
        default=int_env("OPENCODE_AGENT_EXEC_TIMEOUT", 900),
        help="OpenCode command timeout in seconds. Defaults to OPENCODE_AGENT_EXEC_TIMEOUT or 900.",
    )
    parser.add_argument(
        "--raw",
        action="store_true",
        help="Stream OpenCode's raw JSONL instead of the concise transcript.",
    )
...
def run_turn(
    ...
    command = shell_join(
        ...
        opencode_command(prompt, mode="json", name=name),
    )
    return run_command(
        sandbox,
        command,
...
def main() -> int:
    ...
    # credentials OpenCode caches under /root/.pi/agent, widening exposure — for
    ...
    try:
        ...
        version_result = run_command(sandbox, f"{opencode_cli()} --version", timeout=60)
        ...
        result_1 = run_turn(
            sandbox, args.workspace, turn_1_prompt, args.name, args.exec_timeout, opencode_env
        )
        ...
        result_2 = run_turn(
            sandbox, args.workspace, turn_2_prompt, args.name, args.exec_timeout, opencode_env
        )
