...
def _render_jsonl_line(line: str) -> None:
    """Render one OpenCode ``--mode json`` event as concise, human-readable output.

    OpenCode streams a lot of envelopes (per-token deltas, thinking traces,
    duplicate message snapshots). We ignore all of those and render only the
    authoritative ``agent_end`` transcript: assistant text, tool calls, and any
    failures, in order. Non-JSON lines are printed verbatim. Set
    OPENCODE_STREAM_RAW=1 (or pass ``--raw``) to see the raw JSONL stream
    instead.
    """
    line = line.rstrip("\r\n")
    if not line.strip():
...
def jsonl_render_writer() -> Callable[[object], None]:
    ...
    # newlines so each event is rendered exactly once. OpenCode newline-
    # terminates every event, so nothing important is left dangling in the
    # buffer.
    buffer = {"text": ""}
...
def run_command(
    ...
    *,
    ...
    # Run as root: /workspace and OpenCode's state dir (/root/.pi/agent) are
    ...
    if stream:
        ...
        # Set OPENCODE_STREAM_RAW=1 (or pass --raw) to dump OpenCode's raw JSONL.
